...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = 'primary', size = 'default', type = 'button', ...props }, ref) => {
    const baseStyles =
      'inline-flex items-center justify-center whitespace-nowrap rounded-[6px] font-medium transition-all duration-150 focus-visible:outline-none focus-visible:ring-[3px] focus-visible:ring-[#0671B8]/25 disabled:pointer-events-none disabled:opacity-50 select-none';
    ...
    const variants = {
      ...
      destructive:
        'bg-transparent border border-[#DC2626]/40 text-[#DC2626] hover:bg-[#DC2626]/10 hover:border-[#DC2626] hover:-translate-y-[1px] active:translate-y-0',
    };

    const sizes = {
      default: 'h-[38px] px-4 text-[14px]',
      sm: 'h-8 px-3 text-[12px] min-h-[36px] sm:min-h-0',
